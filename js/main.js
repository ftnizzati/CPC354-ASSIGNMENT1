...
function main() {
  ...
  window.updateTextExtrusion = function(newDepth) {
    ...
    positions = [];
    buildLetterF(startX, startY, newDepth, positions);
    buildLetterI(startX + w + spacing, startY, newDepth, positions);
    buildLetterT(startX + (w + spacing) * 2, startY, newDepth, positions);
